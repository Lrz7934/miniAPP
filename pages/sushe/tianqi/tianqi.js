...
const devicesId = "658162260" // 填写在OneNet上获得的devicesId 形式就是一串数字 例子:9939133
const api_key = "TmNsGbKf0akIscVwyPaSjcD=aGw=" // 填写在OneNet上的 api-key 例子: VeFI0HZ44Qn5dZO14AuLbWSlSlI=
...
Page({
  ...
  getDatapoints: function () {
    return new Promise((resolve, reject) => {
      wx.request({
        url: `https://api.heclouds.com/devices/${devicesId}/datapoints?datastream_id=Temperature,Humidity,VAL,Light&limit=20`,
        /**
         * 添加HTTP报文的请求头, 
        ...
        success: (res) => {
          ...
          resolve({
            val: response.data.datastreams[0].datapoints.reverse(),
          ...
          this.setData({
            val:response.data.datastreams[0].datapoints[19].value
          })
        },
        //val:response.data.datastreams[0].datapoints[19],
        fail: (err) => {
          reject(err)
        }
      })
